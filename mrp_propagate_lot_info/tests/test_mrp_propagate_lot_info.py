...
class TestMrpPropagateLotInfo(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.neutral_category = cls.env["product.category"].create(
            {"name": "No lot propagation category"}
        )
        cls.finished_product = cls.env["product.product"].create(
            {
                "name": "Lot Propagation Finished Product",
                "is_storable": True,
                "categ_id": cls.category.id,
                "tracking": "lot",
            }
        )
        cls.manual_finished_product = cls.env["product.product"].create(
            {
                "name": "Manual Lot Propagation Finished Product",
                "is_storable": True,
                "categ_id": cls.category.id,
                "tracking": "lot",
            }
        ...
        cls.component = cls.env["product.product"].create(
            {
                "name": "Lot Propagation Component",
                "is_storable": True,
                "categ_id": cls.category.id,
                "tracking": "lot",
            }
        )
        cls.neutral_component = cls.env["product.product"].create(
            {
                "name": "Component Without Lot Propagation",
                "is_storable": True,
                "categ_id": cls.neutral_category.id,
                "tracking": "lot",
            }
        )
        cls.other_component = cls.env["product.product"].create(
            {
                "name": "Other Lot Propagation Component",
                "is_storable": True,
                "tracking": "lot",
            }
        )
    ...
    @classmethod
    def _create_bom(
        cls, propagate_fields=True, byproduct_propagate=None, component=None
    ):
        component = component or cls.component
        byproduct_values = []
    ...
    def test_confirm_bom_without_propagation_does_not_use_product_category_fields(self):
        bom = self._create_bom(propagate_fields=False, component=self.neutral_component)
        order = self._create_order(bom)
        order.action_confirm()
        self.assertFalse(order.propagate_lot_bom_line_id)

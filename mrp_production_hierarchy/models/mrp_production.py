...
class MrpProduction(models.Model):
    ...
    root_id = fields.Many2one(
        "mrp.production", u"Root order", index=True, ondelete="restrict", readonly=True
    )
    ...
    def action_confirm(self):
        ...
        for prod in self:
            ...
            if not prod.env.context.get("root_mrp_production_id"):
                prod = prod.with_context(root_mrp_production_id=prod.id)
            # Set the parent production order ID
            prod = prod.with_context(parent_mrp_production_id=prod.id)
        ...
        if self.child_ids:
            return {
                ...
                "views": [
                    (
                        self.env.ref(
                            "mrp_production_hierarchy.mrp_production_tree_view"
                        ).id,
                        "tree",
                    ),
                    (
                        self.env.ref(
                            "mrp_production_hierarchy.mrp_production_form_view"
                        ).id,
                        "form",
                    ),
                ],

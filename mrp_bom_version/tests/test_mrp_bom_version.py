...
class TestMrpBomVersion(common.TransactionCase):
    ...
    def test_mrp_bom_back2draft_default(self):
        ...
        # Condition are True
        self.assertTrue(self.mrp_bom.active, "Check must be False")

    def test_mrp_bom_back2draft_active(self):
        # A new key is created because the old key is already created
        self.parameter_model.sudo().create({"key": "active_drafts", "value": True})
        self.mrp_bom.button_activate()
        self.mrp_bom.button_draft()

...
class StockPicking(models.Model):
    ...
    def _compute_display_action_record_components(self):
        for picking in self:
            ...
            if all(
                subcontracted_move.has_tracking == "none"
            ...
            if not subcontracted_productions.filtered(
                lambda mo: (not mo.check_to_done and mo.state != "done")
            ):
                picking.display_action_record_components = False
                continue
    ...
    def action_done(self):
        ...
        for subcontracted_production in productions:
            ...
            minimum_date = datetime.strptime(
                min(picking.move_line_ids.mapped("date")),
                DEFAULT_SERVER_DATETIME_FORMAT,
            )
            production_moves = (
                subcontracted_production.move_raw_ids
                | subcontracted_production.move_finished_ids
            )
            production_moves.write(
                {
                    "date": datetime.strftime(
                        minimum_date - timedelta(seconds=1),
                        DEFAULT_SERVER_DATETIME_FORMAT,
                    ),
                }
            )
    ...
    def _prepare_subcontract_mo_vals(self, subcontract_move, bom):
        ...
        vals = {
            ...
            "bom_id": bom.id,
            "location_src_id": subcontract_move.picking_id.partner_id.with_context(
                force_company=subcontract_move.company_id.id
            ).property_stock_subcontractor.id,
            "location_dest_id": subcontract_move.picking_id.partner_id.with_context(
                force_company=subcontract_move.company_id.id
            ).property_stock_subcontractor.id,
    ...
    def _subcontracted_produce(self, subcontract_details):
        ...
        for move, bom in subcontract_details:
            mo = (
                ...
                .with_context(force_company=move.company_id.id)

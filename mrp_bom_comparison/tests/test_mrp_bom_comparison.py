...
class TestMrpBomComparison(TestMrpCommon):
    def setUp(self):
        ...
        self.new_bom_2.bom_line_ids.filtered(
            lambda line: line.product_id == self.product_4
        ).write({"product_id": self.new_product_4.id})
        ...
        self.new_bom_2.bom_line_ids.filtered(
            lambda line: line.product_id == self.new_product_4
        ).product_qty = 6
        ...
        self.new_bom_3.bom_line_ids.filtered(
            lambda line: line.product_id == self.product_5
        ).write({"product_id": self.new_product_5.id})
    ...
    def test_line_added_ids(self):
        ...
        line_new_product_5 = self.wiz.line_added_ids.filtered(
            lambda line: line.product_id == self.new_product_5
        )

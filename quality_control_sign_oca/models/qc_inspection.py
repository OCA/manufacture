...
class QcInspection(models.Model):
    ...
    def _get_sign_report(self):
        ...
        if not report_id:
            raise UserError(
                self.env._("No Inspection Report configured for signature in Settings.")
            )
        ...
        if not report or not report.exists():
            raise UserError(
                self.env._(
                    "Configured inspection report not found or has been deleted."
                )
            )
    ...
    def action_sign_inspection(self):
        ...
        request_items = []
        for role, _ in slots_by_role.items():
        ...
        if not request_items:
            raise UserError(self.env._("No signers found for this inspection."))

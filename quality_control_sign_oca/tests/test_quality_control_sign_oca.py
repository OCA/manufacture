...
class TestQualityControlSignOca(TestQualityControlOcaBase):
    ...
    def _get_valid_signature_base64(self):
        """Generate a valid base64-encoded PNG signature for testing."""
        minimal_png = (
    ...
    def _build_sign_data(self, signer):
        ...
        signature_b64 = self._get_valid_signature_base64()

        data = {}
        for key, val in items.items():
            item = val.copy()
            if item.get("field_type") == "signature":
                item["value"] = signature_b64
            else:

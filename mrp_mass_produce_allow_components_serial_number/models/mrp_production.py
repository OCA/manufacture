...
class MrpProduction(models.Model):
    ...
    def _check_serial_mass_produce_components(self):
        ...
        # override has_serial_components value because if this module is
        # installed, we want to be able to mass produce with components
        # tracked by S/N and not only by lot
    ...
    def _split_productions(  # noqa: C901
        self,
        amounts=False,
        cancel_remaning_qty=False,
        set_consumed_qty=False,
    ):
        """Complete override to have possibility to manage split w/ components
        of finished product managed by SN.
        """
        ...
        for initial_move, backorder_moves in move_to_backorder_moves.items():
            ...
            for quantity, move_line, ml_vals in ml_by_move:
                while (
                    ...
                    # Do not create `stock.move.line` if there is no
                    # initial demand on `stock.move`
        ...
        # ## START CUSTOMIZATION FOR S/N HANDLING
        if any(move.product_id.tracking == "serial" for move in self.move_raw_ids):
            # manage move re-assign
            self.env["stock.move"].browse(assigned_moves)._recompute_state()
            self.env["stock.move"].browse(partially_assigned_moves)._recompute_state()
        ...
        # ## END CUSTOMIZATION FOR S/N HANDLING

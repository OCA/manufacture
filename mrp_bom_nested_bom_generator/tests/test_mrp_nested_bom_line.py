...
class TestMrpNestedBomLine(TestNestedBomCase):
    def setUp(self):
        super(TestMrpNestedBomLine, self).setUp()
        ...
        self.mrp_nested_bom_line_log = self.env["mrp.nested.bom.line"].create(
            {
    ...
    def test_onchange_product_tmpl_id(self):
        self.assertEqual(
            len(self.mrp_nested_bom_line_wood.attribute_ids),
            0,
            msg="Attributes count must be equal to zero",
        )
        self.mrp_nested_bom_line_wood.write(
            {"product_tmpl_id": self.product_template_log.id}
        )
        self.mrp_nested_bom_line_wood._onchange_product_tmpl_id()
        self.assertEqual(
            len(self.mrp_nested_bom_line_wood.attribute_ids),
            1,
    ...
    def test_compute_attribute_aggregated_ids(self):
        self.assertEqual(
            len(self.mrp_nested_bom_line_log.attribute_aggregated_ids),
            2,
    ...
    def test_create_product(self):
        MrpNestedBomLine = self.env["mrp.nested.bom.line"]
        result = MrpNestedBomLine.create_product(MrpNestedBomLine)
        self.assertFalse(result, msg="Result must be False")
        bom = self.mrp_bom_pinocchio_mrp
        result_product_template_id = MrpNestedBomLine.create_product(bom)
        ...
        self.env["mrp.nested.bom.line"].create(
            {
                "bom_id": bom.id,
        ...
        result_product_template = MrpNestedBomLine.create_product(bom)
        self.assertEqual(
    ...
    def test_create_valid(self):
        result = self.env["mrp.nested.bom.line"].create(
    ...
    def test_prepare_bom_product_tmpl_attribute_ids(self):
        result = self.mrp_nested_bom_line_wood._prepare_bom_product_tmpl_attribute_ids()
        self.assertEqual(result, set(), msg="Result must be empty set")
        self.mrp_nested_bom_line_wood.attribute_ids = [
            (4, self.product_attribute_size.id)
        ]
        result = self.mrp_nested_bom_line_wood._prepare_bom_product_tmpl_attribute_ids()
        self.assertEqual(
            result,
    ...
    def test_find_product_template_attributes(self):
        result = self.mrp_nested_bom_line_log._find_product_template_attributes([])
    ...
    def test_prepare_product_attribute(self):
        ...
        mrp_nested_ids = self.env["mrp.nested.bom.line"].browse(
            [self.mrp_nested_bom_line_log.id, self.mrp_nested_bom_line_wood.id]
        )

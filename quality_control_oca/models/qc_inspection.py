...
class QcInspection(models.Model):
    ...
    def _unlink_except_autogenerated_and_non_draft(self):
        ...
        if not self.env.is_superuser():
            auto_generated = self.filtered("auto_generated")
            if auto_generated:
                errors.append(
                    _("- Auto-generated inspections: %s")
                    % ", ".join(auto_generated.mapped("display_name"))
                )
        non_draft = self.filtered(lambda r: r.state != "draft")
        if non_draft:
            errors.append(
                _("- Inspections not in draft state: %s")
                % ", ".join(non_draft.mapped("display_name"))
            )
        if errors:
            raise exceptions.UserError(
                _("The following inspections cannot be deleted:\n\n%s")
                % "\n".join(errors)
            )

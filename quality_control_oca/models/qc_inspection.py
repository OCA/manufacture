...
class QcInspection(models.Model):
    ...
    def _unlink_except_autogenerated_and_non_draft(self):
        ...
        non_deletable_records = self.filtered(
            lambda flt: flt.auto_generated or flt.state != "draft"
        )

        if len(non_deletable_records) > 0:
            raise exceptions.UserError(
                self.env._(
                    "Inspections %s are auto-generated or not in draft "
                    "state and can only be deleted by superuser.",
                    ", ".join(non_deletable_records.mapped("display_name")),
                )
            )

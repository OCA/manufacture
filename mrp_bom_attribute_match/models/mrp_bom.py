    ...
    def _get_component_template_product(
        ...
        if bom_line.component_template_id:
            ...
            for ptav in bom_product_id.product_template_attribute_value_ids:
                # Only match on attributes the component actually has, and skip
                # archived component values (ptav_active is not the magic
                # `active` field, so search does not filter it out): an archived
                # value that happens to share the product's value would build an
                # impossible combination and wrongly discard the component.
                if ptav.attribute_id.id not in comp_attr_ids:
                    continue

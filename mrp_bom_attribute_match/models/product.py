...
class ProductTemplate(models.Model):
    ...
    def _check_component_change_allowed(self):
        for rec in self:
            ...
            for bom in boms:
                ...
                comp_attr_ids = rec.attribute_line_ids.attribute_id.ids
                diff_ids = list(set(comp_attr_ids) - set(prod_attr_ids))

                if diff_ids:
                    attr_recs = rec.env["product.attribute"].browse(diff_ids)
    ...
    def _get_component_boms(self):
        ...
        return bom_lines.mapped("bom_id") if bom_lines else False

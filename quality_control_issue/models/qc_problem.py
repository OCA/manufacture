...
class QcProblem(models.Model):
    ...
    issue_ids = fields.Many2many(
        ...
        string="QC Issues",
        relation="qc_issue_problem_rel",
        column1="qc_problem_id",
        column2="qc_issue_id",
    )
    problem_group_id = fields.Many2one(
        comodel_name="qc.problem.group", string="Problem Group"
    )
    issue_count = fields.Integer(string="Issues", compute=_compute_count, store=True)
    color = fields.Integer(string="Color Index")
    priority = fields.Selection(
        selection=AVAILABLE_PRIORITIES, default="0", string="Rating", index=True,
    )
    stage_id = fields.Many2one(
        comodel_name="qc.stage",
        string="Stage",
    ...
    qc_team_id = fields.Many2one(
        comodel_name="qc.team",
        string="QC Team",
        default=lambda self: self.env["qc.team"]
        .sudo()
        ._get_default_qc_team_id(user_id=self.env.uid),
        index=True,
        track_visibility="onchange",
    )
    ...
    def stage_find(self, cases, team, domain=None, order="sequence"):
        ...
        stage = self.env["qc.stage"].search(search_domain, order=order, limit=1)
        return stage
